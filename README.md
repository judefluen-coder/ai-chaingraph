...
| **4,537** 家公司 / 证券 | **111** 个产业环节 | **6,609** 条公司产业映射 | **93.3%** 公司具备具体动作关系 |
| ---: | ---: | ---: | ---: |
| A 股与美股 | 分布于 12 条产业链 | 5,984 条为具体关系 | 4,233 家可定位到生产、开发、运营、集成、提供等动作 |

公开快照版本为 **v1.2.0**。关系层最近深化于 **2026-09-07**，已收录来源截止 **2026-07-12**；项目按周检查新增披露，并把“检查时间”和“数据实际变化时间”分开记录。
...
覆盖广度只有在关系足够具体、来源可以复核时才有意义。v1.2 将具备具体动作关系的公司从 **1,149 家（25.3%）**提升到 **4,233 家（93.3%）**，同时保留 304 家证据不足的公司为宽口径映射，而不是用推断补齐。
...
| 公司产业映射 | 6,609 | 5,984 条具体动作关系，625 条宽口径映射 |
...
```mermaid
flowchart LR
    A["监管文件、交易所公告、公司正式披露"] --> B["候选关系与来源复核"]
    B --> C["审核批次 data/weekly-candidates"]
    C --> D["导入与关系类型化"]
    D --> E["快照完整性校验"]
    E --> F["关系深度与时效审计"]
...
- **每周一 09:00（Asia/Shanghai）**：检查上一周期新增的官方披露，优先处理宽口径公司和低覆盖产业环节。
- **证据准入**：新增关系必须有公开 HTTPS 来源、可复核事实声明、明确动作和 L1 / L2 等级。
- **发布门槛**：测试、快照校验、关系深度审计和生产构建必须全部通过。
- **质量守门**：GitHub Actions 要求最近检查时间不超过 8 天，具备具体关系的公司比例不低于 90%。
