...
- 产业链路径对比：公司详情中展示当前公司路径、同链路可比公司、共享节点和差异节点。
- 质量提示：提示跨市场覆盖缺口、待审核映射、证据时效、证据强弱不一和证据冲突。
...
│   ├── fictional-ai-mappings-duplicates.csv
...
- 数据层：补齐真实数据去重、证据冲突批量修复和 L3 审核工作流。
- API 层：补齐更完整的持久化、权限边界和增量更新工作流。
- 研究体验：增加跨市场路径对比、同行分组和更完整的证据冲突处理。
