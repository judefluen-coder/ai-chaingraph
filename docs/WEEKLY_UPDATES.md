...
- Monday 09:00 Asia/Shanghai: inspect official disclosures published since the previous source cutoff.
...
npm run audit:relationships -- --min-specific-issuer-rate=0.9
npm run build
```

`public/snapshots/update-status.json` records the latest check, next scheduled check, source cutoff, imported batches, promotions, and current relationship-quality metrics.
